...
export class HighlightPipe implements OnChanges, AfterViewInit, AfterViewChecked {
  ...
  private markMatchedTextAndRemoveUniqueTokens(htmlNode: HTMLElement, searchRegex: RegExp) {
    if (htmlNode.innerHTML) {
      ...
      const newHtml = innerHtml.replace(searchRegex, match => {
        ...
        return `<strong>${markedStr}</strong>`;
      });
  ...
  private removePreviouslyMarkedTextInNode() {
    ...
    const markingPattern = new RegExp('<strong>|<\/strong>', 'g');
  ...
  //   const markedText = node.innerHTML.replace(searchRegex, '<strong>$&</strong>');
